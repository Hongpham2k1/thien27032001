...
public class RetrCommand {
	...
	public String downLoadFile(DataInputStream controldis, DataOutputStream controldos, String options[],
			...
			if (serverMsg.equals("File not found on the server")) {
				...
				if (!fileCheckedOnServer && file.exists()) {
					...
					if (options.length == 1 || (options.length > 1 && options[1] == "Y")) {
						...
						Double v = Double.parseDouble(fileSize);
						...
						do {
							...
							if (ch != -1) {
								...
								jProgressBar
										.setString(options[0] + " downloading... " + val*100 / v.intValue() + "%");
							}
